    ...
    const fetchAchievements = async () => {
      try {
        ...
        if (Array.isArray(res.data) && res.data.length > 0) {
          ...
          setAchievements(achievementsMock);
        }
      } catch (err) {
        ...
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="text-xs text-text-muted max-w-xl">
              Rekap prestasi siswa XI RPL di berbagai bidang. Data ini sudah disiapkan
              untuk diambil dari backend, dengan filter dinamis di sisi frontend.
            </p>
          </div>
              ...
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2 text-xs text-text-muted">
                ...
                {years.length > 0 && (
                  <p className="text-[11px]">
                    Tahun terbaru{' '}
                    <span className="font-semibold text-text">{years[0]}</span> memiliki{' '}
                    <span className="font-semibold text-text">
                      {totalByYear(years[0])}
                    </span>{' '}
                    prestasi.
